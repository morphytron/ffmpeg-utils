...
class create_mp3_from_folder_and_subdirectories:
    ...
    def get_args(self):
        ...
        while True:
            try:
                nx = next(args_iter)
                if nx == '-C' or nx == '--convert-to-ext':
                    try:
                        self.convert_to_file_ext = next(args_iter)
                    except:
                        print("Possibly no argument following -C, '--convert-to-ext'")
                elif nx == '-h' or nx == '--help':
                    print("-C or --convert-to-ext argument typically should be of 'mp3' format, however, it could be a different audio format supported by ffmpeg.")
        ...
        for key in all_media_files.keys():
            ...
            with open(list_of_vids, 'w') as f:
                for abs_path_fname in all_media_files[key]:
                    write_line = 'file \'{}.{}\'\n'.format(abs_path_fname[:-(file_ext_len + 1)], self.convert_to_file_ext)
    ...
    def execute_transcode(self, all_media_files={}, all_key_abs_paths={}, file_ext='', file_ext_len=0):
        for key in all_media_files.keys():
            ...
            for abs_path_to_mp4 in all_media_files[key]:
                ...
                out_args.append("\"{}\"".format(abs_path_to_mp4[:-(file_ext_len + 1)] + '.{}'.format(self.convert_to_file_ext)))
                index += 1
            while len(in_args) > 0:
            ...
            args_last = " ".join(['ffmpeg', '-f', 'concat', '-safe', '0', '-i', "\"{}\"".format(self.all_key_abs_paths[key]), "\"{}\"".format(self.all_key_abs_paths[key][:-(file_ext_len + 1)] + '.{}'.format(self.convert_to_file_ext))])
            print(args_last)
            sp.run(args_last,
